...
def save_info_data(data):
    with open(f'{data["origin"]}.info', 'w+') as file:
        json.dump(data, file)
...
def get_records_from_answer(answer, count):
    ...
    for _ in range(count):
        ...
        ptr += 12 + rd_length
        rec_type = Controller.make_type_from_number(rec_type)
        record['ttl'] = ttl
...
def get_info(domain, info_data, qtype):
    ...
    if info_name in info_data:
        print(f'Данные по {info_name} обнаружены.')
        ...
        if qtype in info['data']:
            ...
            if (current_time - time).seconds > ttl:
                print(f'Данные по "{info_name}" посторели. Вызываю старшего.')
                info = find_data(domain, qtype)
        else:
            print(f'Данные по "{qtype}" не обнаруженны. Вызываю старшего.')
            info = find_data(domain, qtype)
    else:
        print(f'В кэше нет данных по "{info_name}". Вызываю старшего.')
        info = find_data(domain, qtype)
...
def get_records(data):
    domain, question_type = Controller.get_question_domain(data)
    QT = ''
...
def make_response(data):
    request_info = Controller.parse_request(data)
    resp = b''
